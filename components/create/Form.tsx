...
const Form = ({numberOfPlaces, setShowForm} : FormProps) => {
    ...
    const [textSearchApiResults, setTextSearchApiResults] = useState<PlaceTextSearchResult[]>([])
    ...
    async function fetchApi(queryValue: string) {
        const response = await fetch(`http://localhost:3000/api/textSearch/${queryValue}`);
        ...
        <div className="bg-white p-6 flex flex-col gap-4 rounded-md shadow-sm">
            <Combobox value={selectedOption} onChange={setSelectedOption}>
                ...
                <div className='flex w-full justify-between'>
                    <p>Place</p>
                    {numberOfPlaces > 0 && <DeleteOutlinedIcon className='cursor-pointer' onClick={() => setShowForm(false)} />}
                ...
                <div className='relative w-full'>
                    ...
                    {textSearchApiResults.length === 0 && queryState !== '' ?
                        (
                            ...
                            queryState !== '' && (
                                <Combobox.Options
                                    className='mx-auto max-w-[336px] rounded-md mt-3 py-2 border-2 bg-white shadow-md w-full flex flex-col text-stone-500'
                                >
                ...
                {chosenPlaceId != null && chosenPlaceId !== '' &&
                    <button onClick={() => handleAddPlace(chosenPlaceId)} className="normalButton">Add this place</button>}
